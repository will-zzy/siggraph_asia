...
def parse_args():
    ...
    parser.add_argument('--show-dir', help='the root dir')
    parser.add_argument('--output-dir', help='the output dir')
    ...
    parser.add_argument('--vis', default=False, type=bool, help='visualize the results')
...
def main(args):
    ...
    root_dir = os.path.dirname(args.test_data_path)
    ...
    if not cfg.distributed:
        main_worker(0, cfg, args.launcher, test_data, root_dir)
    else:
        ...
        if args.launcher == 'ror':
            ...
            mp.spawn(main_worker, nprocs=cfg.dist_params.num_gpus_per_node, args=(cfg, args.launcher, test_data, root_dir))
        
def main_worker(local_rank: int, cfg: dict, launcher: str, test_data: list, root_dir: str):
    if cfg.distributed:
