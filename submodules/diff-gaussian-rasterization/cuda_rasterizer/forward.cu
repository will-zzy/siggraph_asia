	...
	bool antialiasing)
{
	...
	float3 conic = { cov.z * det_inv, -cov.y * det_inv, cov.x * det_inv }; // 2d协方差的逆
	...
	// float4 con_o = {conic.x, conic.y, conic.z, opacity} // TODO:check if need filter
	float4 con_o = {conic.x, conic.y, conic.z, opacity * h_convolution_scaling};
	uint32_t tiles_count = duplicateToTilesTouched( // 此处tiles_count是裁剪后的
	...
	bool prefiltered)
{
	...
	filter_preprocessCUDA<NUM_CHANNELS_3DGS> << <(P + 255) / 256, 256 >> > (
		P, M,
		means3D,
		scales,
		scale_modifier,
...
renderSimpCUDA(
	...
	uint32_t* __restrict__ n_contrib,
	uint32_t* __restrict__ max_contrib,
	float* __restrict__ accum_weights_ptr,
	float* __restrict__ accum_weights_count,
	float* __restrict__ accum_max_count,
	const float* __restrict__ bg_color,
	float* __restrict__ out_color,
	const float* __restrict__ depths,
	float* __restrict__ invdepth)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			atomicAdd(&(accum_weights_ptr[collected_id[j]]), alpha * T); // 每个高斯对所有像素的权重之和
			atomicAdd(&(accum_weights_count[collected_id[j]]), 1); // 每个高斯的投影面积
	...
	if(flag_update==1)
	{
		atomicAdd(&(accum_max_count[idx_max]), 1); // 每个高斯为最大贡献者的计数器
	}
...
void FORWARD::render_simp(
	...
	float* depth)
{
	renderSimpCUDA<NUM_CHANNELS_3DGS> << <grid, block >> > (
		ranges,
		point_list,
		per_tile_bucket_offset, bucket_to_tile,
		sampled_T, sampled_ar, sampled_ard,
